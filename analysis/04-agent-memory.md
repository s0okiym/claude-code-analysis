...
```text
主会话持续进行
  -> 检查是否达到 token / tool call 阈值
     -> 否：继续主会话
     -> 是：
        1. 触发 post-sampling hook
        2. 拉起 forked subagent
        3. 读取当前 session memory 文件
        4. 按模板更新 markdown 摘要
        5. 写回 session memory 文件
        6. 后续 compact 优先利用该摘要
```
...
```text
项目内 snapshot
  (.claude/agent-memory-snapshots/<agent>/)
    -> checkAgentMemorySnapshot()
       -> 本地无记忆：initializeFromSnapshot()
       -> snapshot 更新：prompt-update
       -> 已同步：不处理

initializeFromSnapshot() / prompt-update
  -> 本地 agent memory 目录
```
...
本地 team memory 文件
  -> hash / checksum
  -> secret scanning / path validation
  -> 按差异 PUT 上传
  -> 服务端 team memory
